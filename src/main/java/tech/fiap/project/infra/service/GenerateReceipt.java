...
public class GenerateReceipt implements GenerateReceiptUseCase {
	...
	public Receipt confirmPayment(Payment payment) {
		...
		try {
			...
			document.add(new Paragraph("Fiap Tech Challenge").setFontSize(20).setFontColor(tituloCor).setBold()
					.setTextAlignment(TextAlignment.CENTER));

			document.add(new Paragraph("Comprovante de Pagamento").setFontSize(16).setBold()
					.setTextAlignment(TextAlignment.CENTER).setMarginBottom(20));

			document.add(new Paragraph("Data e Hora: " + dataHoraFormatada).setFontSize(12).setFontColor(textoCor)
					.setTextAlignment(TextAlignment.LEFT).setMarginBottom(10));

			document.add(new Paragraph("Valor Pago: R$ " + String.format("%.2f", payment.getAmount())).setFontSize(12)
					.setFontColor(textoCor).setTextAlignment(TextAlignment.LEFT).setMarginBottom(10));

			document.add(new Paragraph("Método de pagamento: ").setFontSize(12).setFontColor(textoCor)
					.add(new Paragraph("Pix").setBold()));

			document.add(
					new Paragraph(String.format("Código de Autenticação: %s", generateHash(payment.getId().toString())))
							.setFontSize(12).setFontColor(textoCor).setTextAlignment(TextAlignment.LEFT)
							.setMarginBottom(10));

			document.add(new Paragraph("Obrigado por utilizar nossos serviços!").setFontSize(12).setFontColor(textoCor)
					.setTextAlignment(TextAlignment.LEFT));

			document.add(new Paragraph("Bom apetite!").setFontSize(14).setBold().setFontColor(tituloCor)
